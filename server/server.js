...
function movePlayer(player) {
    ...
    for (var i = 0; i < player.cells.length; i++) {
        ...
        if (player.cells[i].speed <= 6.25) {
            slowDown =
                util.log(player.cells[i].mass, c.slowBase) - initMassLog + 1;
        }
        ...
        for (var j = 0; j < player.cells.length; j++) {
            if (j != i && player.cells[i] !== undefined) {
                ...
                var radiusTotal =
                    player.cells[i].radius + player.cells[j].radius;
                if (distance < radiusTotal) {
                    if (
                        player.lastSplit >
                        new Date().getTime() - 1000 * c.mergeTimer
                    ) {
                        ...
                        player.cells[i].radius = util.massToRadius(
                            player.cells[i].mass
                        );
    ...
    socket.on("pass", function(data) {
        if (data[0] === c.adminPass) {
            ...
            socket.broadcast.emit(
                "serverMSG",
                currentPlayer.name + " just logged in as admin!"
            );
        ...
        for (var i = 0; i < currentPlayer.cells.length; i++) {
            if (
                (currentPlayer.cells[i].mass >=
                    c.defaultPlayerMass + c.fireFood &&
                    c.fireFood > 0) ||
